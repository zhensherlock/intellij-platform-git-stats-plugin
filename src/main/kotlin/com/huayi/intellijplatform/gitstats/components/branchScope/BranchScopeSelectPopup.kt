...
internal object BranchScopeSelectPopup {
    fun show(
        anchor: JComponent,
        branchInfo: BranchInfo,
        selectedScope: BranchScope,
        applyScope: (BranchScope) -> Unit,
        onClosed: () -> Unit
    ): JBPopup? {
        val branches = branchInfo.selectableBranches
        if (branches.isEmpty()) {
            return null
        }

        val input = JBTextArea(initialText(branchInfo, selectedScope), TEXT_ROWS, TEXT_COLUMNS).apply {
            lineWrap = false
            border = JBUI.Borders.empty(6, 6)
            focusTraversalKeysEnabled = false
        }
        val listModel = DefaultListModel<BranchRef>()
        val suggestionList = JBList(listModel).apply {
            selectionMode = ListSelectionModel.SINGLE_SELECTION
            visibleRowCount = MAX_VISIBLE_ROWS
            cellRenderer = BranchRefRenderer { activeToken(input.text, input.caretPosition) }
            emptyText.text = MyBundle.message("branchScopeSelectNoMatches")
        }
        val suggestionScrollPane = JBScrollPane(suggestionList).apply {
            border = JBUI.Borders.emptyTop(1)
            preferredSize = Dimension(JBUI.scale(420), JBUI.scale(suggestionHeight(branches.size)))
        }
        ...
        fun applyText() {
            val resolvedBranches = resolveBranchRefs(input.text, branches)
                ?: suggestionList.selectedValue?.let { selected ->
                    val replacement = replaceActiveToken(input.text, input.caretPosition, selected.name)
                    input.text = replacement.text
                    input.caretPosition = replacement.caretPosition.coerceIn(0, input.text.length)
                    resolveBranchRefs(replacement.text, branches)
                }
                ?: emptyList()
            BranchScope.selectedBranches(resolvedBranches)?.let { scope ->
                applyScope(scope)
                popup?.closeOk(null)
            } ?: run {
        ...
        fun completeFromSuggestion(appendNewLine: Boolean = false) {
            ...
            selectedValue.let { selected ->
                val replacement = if (appendNewLine) {
                    replaceActiveTokenAndAppendNewLine(input.text, input.caretPosition, selected.name)
                } else {
                    replaceActiveToken(input.text, input.caretPosition, selected.name)
                }
                input.text = replacement.text
                input.caretPosition = replacement.caretPosition.coerceIn(0, input.text.length)
                refreshSuggestions()
        ...
        val content = JBPanel<JBPanel<*>>(BorderLayout()).apply {
            ...
            add(JBLabel(MyBundle.message("branchScopeSelectHint")).apply {
        ...
        popup = JBPopupFactory.getInstance()
            ...
            .setMinSize(Dimension(JBUI.scale(420), JBUI.scale(170)))
            .createPopup()
    ...
    private fun initialText(branchInfo: BranchInfo, selectedScope: BranchScope): String {
        return when (selectedScope) {
            is BranchScope.SelectedBranch -> selectedScope.branchName
            is BranchScope.SelectedBranches -> selectedScope.branches.joinToString(BRANCH_JOINER) { it.branchName }
            else -> branchInfo.currentBranch.orEmpty()
        }
    }

    internal fun activeToken(text: String, caretPosition: Int): String {
        val range = activeTokenRange(text, caretPosition)
        return text.substring(range.start, range.end)
    }

    internal fun replaceActiveToken(text: String, caretPosition: Int, replacement: String): TokenReplacement {
        val range = activeTokenRange(text, caretPosition)
    ...
    internal fun replaceActiveTokenAndAppendNewLine(
        text: String,
        caretPosition: Int,
        replacement: String
    ): TokenReplacement {
    ...
    internal fun insertNewLine(text: String, caretPosition: Int): TokenReplacement {
        val caret = caretPosition.coerceIn(0, text.length)
        return TokenReplacement(
            text.take(caret) + "\n" + text.drop(caret),
            caret + 1
    ...
    private fun resolveBranchRefs(text: String, branches: List<BranchRef>): List<BranchRef>? {
        val tokens = text
            .split('|', '\n')
            .map { it.trim() }
        ...
        return tokens.map { token ->
            branches.firstOrNull { it.name.equals(token, ignoreCase = false) }
                ?: branches.firstOrNull { it.name.equals(token, ignoreCase = true) }
                ?: return null
        }
    }
    ...
    private fun installKeyboardActions(
        input: JBTextArea,
        suggestionList: JBList<BranchRef>,
        completeFromSuggestion: () -> Unit,
        completeFromSuggestionAndNewLine: () -> Unit,
        applyText: () -> Unit
    ) {
        input.inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0), "gitStatsNextBranchSuggestion")
        input.actionMap.put("gitStatsNextBranchSuggestion", object : AbstractAction() {
            override fun actionPerformed(e: java.awt.event.ActionEvent) {
                moveSelection(suggestionList, 1)
            }
        })
        input.inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0), "gitStatsPreviousBranchSuggestion")
        input.actionMap.put("gitStatsPreviousBranchSuggestion", object : AbstractAction() {
            override fun actionPerformed(e: java.awt.event.ActionEvent) {
                moveSelection(suggestionList, -1)
            }
        })
        input.inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_TAB, 0), "gitStatsCompleteBranchSuggestion")
        input.actionMap.put("gitStatsCompleteBranchSuggestion", object : AbstractAction() {
            override fun actionPerformed(e: java.awt.event.ActionEvent) {
                completeFromSuggestion()
            }
        })
        input.inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "gitStatsCompleteBranchSuggestionAndNewLine")
        input.actionMap.put("gitStatsCompleteBranchSuggestionAndNewLine", object : AbstractAction() {
            override fun actionPerformed(e: java.awt.event.ActionEvent) {
        ...
        input.inputMap.put(
            ...
            "gitStatsApplyBranchSelection"
        )
        input.inputMap.put(
            KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, InputEvent.CTRL_DOWN_MASK),
            "gitStatsApplyBranchSelection"
        )
        input.actionMap.put("gitStatsApplyBranchSelection", object : AbstractAction() {
            override fun actionPerformed(e: java.awt.event.ActionEvent) {
                applyText()
            }
        })
    ...
    private fun moveSelection(list: JBList<BranchRef>, delta: Int) {
    ...
    private fun setSuggestionMenuVisible(
        ...
        if (suggestionScrollPane.isVisible == visible) {
            return
        }
    ...
    private fun suggestionHeight(branchCount: Int): Int {
        return min(branchCount.coerceAtLeast(1), MAX_VISIBLE_ROWS) * SUGGESTION_ROW_HEIGHT
    }

    private class BranchRefRenderer(
        ...
        override fun getListCellRendererComponent(
            ...
            val branchRef = value as? BranchRef ?: return component
            component.icon = AllIcons.Vcs.Branch
            component.text = highlightedName(branchRef.name, query())
    ...
    private const val TEXT_COLUMNS = 42
    ...
    internal data class TokenReplacement(
        val text: String,
